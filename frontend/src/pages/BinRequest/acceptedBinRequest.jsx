...
export default function Item() {
    ...
    useEffect(() => {
        function getItem() {
            axios.get("http://localhost:5000/api/binRequest/getbin")
                .then((res) => {
    ...
    const deleteItem = async (id) => {
        try {
            await axios.delete(`http://localhost:5000/api/binRequest/deletebin/${id}`);
        ...
        <div style={{ margin: '0 auto', maxWidth: '1600px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {item.map((item, index) => (
                <div key={index} style={{ backgroundColor: '#fff', borderRadius: '10px', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)', padding: '20px', marginBottom: '20px', width: 'calc(100% - 50px)', maxWidth: '800px', boxSizing: 'border-box', border: '2px solid #e0e0e0', transition: 'transform 0.3s ease', position: 'relative',height:'270px' }}>
                    ...
                    <h2 style={{ textAlign: '', color: '#333', borderBottom: '1px solid #e0e0e0', paddingBottom: '10px', marginBottom: '10px', border: '1px solid #000000', borderRadius: '10px', padding: '10px', display: 'inline-block',width: '200px' }}><div style={{marginLeft:'19%',color:'green'}}>ACCEPTED</div></h2>
